...
- 검증-재시도 루프를 구현한다.
- 자기 리뷰 vs 독립 인스턴스 리뷰 차이를 관찰한다. (Ch14)
...
| 파일 | 다루는 것 |
|---|---|
| `extract_structured.py` | `tool_use` + nullable 스키마로 환각 방지 (교재 A4 #3) |
| `few_shot_accuracy.py` | few-shot 2개로 모호한 분류의 일관성 개선 관찰 |
| `validate_retry_loop.py` | 스키마 검증 실패 → `tool_result`(is_error)로 재시도 |
| `review_pattern_demo.py` | 자기 리뷰 vs 독립 인스턴스 리뷰 (Ch14, 샘플 37·38) |
...
| `review_pattern_demo_agentsdk.py` | "독립 인스턴스"를 진짜 **서브에이전트**(Task)로 구현 — 코디네이터 대화를 상속 안 하는 게 구조적으로 보장됨(D1 05.3) |
...
- 리뷰는 자기 리뷰 < 독립 인스턴스 리뷰, 멀티패스는 개별 파일 → 크로스파일 순
